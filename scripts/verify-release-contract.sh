...
for property in Version AssemblyVersion FileVersion; do
    ...
    if [[ "${property_value}" != "${version}" ]]; then
        fail "Directory.Build.props ${property} ${property_value:-<missing>} does not match build.yaml version ${version}."
    fi
done

project_framework="$(xml_value TargetFramework "${project_root}/Jellyfin.Plugin.CollectionTagSync/Jellyfin.Plugin.CollectionTagSync.csproj")"
if [[ "${project_framework}" != "${framework}" ]]; then
...
for package_name in Jellyfin.Common Jellyfin.Controller Jellyfin.Model; do
    ...
    if [[ "${pinned_version}" != "${jellyfin_package_version}" ]]; then
        fail "${package_name} ${pinned_version:-<missing>} does not match target ABI package line ${jellyfin_package_version}."
    fi
done

test_server_version="$(sed -n 's|^[[:space:]]*image:[[:space:]]*jellyfin/jellyfin:\([^[:space:]]*\).*|\1|p' \
    "${project_root}/compose.yaml")"
if [[ "${test_server_version}" != "${jellyfin_package_version}" ]]; then
...
if [[ "${framework}" != "net${sdk_major}.0" ]]; then
...
if [[ "${package_target_abi}" != "${target_abi}" ]]; then
